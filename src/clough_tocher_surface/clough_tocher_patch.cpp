  ...
  const std::array<Eigen::Vector3d, 19>& lagrange_nodes)
{
  ...
  // TODO: make global
  std::array<std::array<int64_t, 10>, 3> local_nodes = {{{{0, 1, 18, 3, 4, 14, 15, 13, 12, 9}},
...
PlanarPoint normalize_domain_point(
...
Eigen::Matrix<double, 3, 1> CloughTocherPatch::CT_eval_normalized(int idx, const double &u,
...
void
CloughTocherPatch::triangulate_normalized(size_t num_refinements,
                               std::array<Eigen::MatrixXd, 3>& V,
                               std::array<Eigen::MatrixXi, 3>& F) const
{
  for (int n = 0; n < 3; ++n)
