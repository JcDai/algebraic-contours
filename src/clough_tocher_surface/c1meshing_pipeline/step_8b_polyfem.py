...
def create_polyfem_json(
    ...
    if not enable_offset:
        c_json = {
            "space": {"discr_order": 3, "basis_type": "Bernstein"},
            "geometry": [
                {
                    "mesh": initial_mesh,
                    "volume_selection": 1,
            ...
            "input": {"data": {"state": "initial_solution.hdf5", "reorder": True}},
        }
    else:
        c_json = {
            "contact": {
...
def create_polyfem_json_amips(
    ...
    print("[{}] ".format(datetime.datetime.now()), "create amips json for polyfem")
    ...
    if not enable_offset:
        c_json = {
            ...
            #     "dirichlet_boundary": [{"id": 1, "value": [0.0, 0.0, 0.0]}]
            # },
            "materials": [{"id": 1, "type": "AMIPS", "use_rest_pose": True}],
            "solver": {
                "contact": {"barrier_stiffness": 1e8},
                "nonlinear": {
                    "first_grad_norm_tol": 0,
...
def create_polyfem_json_amips_hpc(
    enable_offset,
    output_name,
    initial_mesh,
    soft_file,
    ...
    with open("constraints_amips_hpc.json", "w") as f:
        json.dump(c_json, f)
...
def call_polyfem(workspace_path, path_to_polyfem_exe, json_file):
    ...
    polyfem_command = (
        path_to_polyfem_exe + " -j " + workspace_path + json_file + " --log_level 1"
    )
...
def resurrect_winding_number(output_name, new_winding_numbers, enable_offset):
    ...
    if enable_offset:
        ...
        new_winding_number_total = np.zeros((len(polyfem_mesh.cells[0]), 1))
