...
def build_bezier_hard_constraint_matrix(
    workspace_path,
    tri_to_tet_index_mapping_file,
    bezier_cons_mat_file,
    linear_tetmesh_file,
):
    print(
...
def build_bezier_reduce2full_matrix(
    workspace_path,
    tri_to_tet_index_mapping_file,
    bezier_reduced2full_file,
    bezier_r2f_col_idx_map_file,
):
    print(
        "[{}] ".format(datetime.datetime.now()),
    ...
    col2local = np.loadtxt(workspace_path + bezier_r2f_col_idx_map_file).astype(
        np.int32
    )
...
def build_expanded_bezier_hard_constraint_matrix(
    workspace_path,
    tri_to_tet_index_mapping_file,
    bezier_cons_mat_file,
    linear_tetmesh_file,
    ...
    for f in bd_f:
        ...
        f_sorted.sort()
        f_str = str(f_sorted[0]) + "+" + str(f_sorted[1]) + "+" + str(f_sorted[2])
    ...
    A_c1_tet_expanded = scipy.sparse.coo_array(
        (A_c1_tet_expanded_data, (A_c1_tet_expanded_row, A_c1_tet_expanded_col)),
        shape=(A_c1.shape[0] * 3, v.shape[0] * 3),
    )
    ...
    dirichlet = scipy.sparse.coo_array(
        (dirichlet_data, (dirichlet_row, dirichlet_col)),
        shape=(bd_v.shape[0], v.shape[0]),
    )
    ...
    dirichlet_expanded = scipy.sparse.coo_array(
        (dirichlet_expanded_data, (dirichlet_expanded_row, dirichlet_expanded_col)),
        shape=(dirichlet.shape[0] * 3, dirichlet.shape[1] * 3),
    )
    ...
    with h5py.File(
        workspace_path + "CT_bezier_constraint_matrix_with_dirichlet_expanded.hdf5", "w"
    ) as f:
    ...
    assert (
        sf_ind_node_ids.shape[0]
        + sf_dep_node_ids.shape[0]
        + other_dep_node_ids.shape[0]
        + other_ind_node_ids.shape[0]
        == v.shape[0]
    )

    # compute full to reduce node id mapping
    all_ind_node_ids = np.sort(np.concatenate((sf_ind_node_ids, other_ind_node_ids)))
    ...
    r2f_full = scipy.sparse.coo_array(
        (r2f_full_data, (r2f_full_row, r2f_full_col)),
        shape=(v.shape[0], all_ind_node_ids.shape[0]),
    )
    ...
    r2f_full_expanded = scipy.sparse.coo_array(
        (r2f_full_expanded_data, (r2f_full_expanded_row, r2f_full_expanded_col)),
        shape=(r2f_full.shape[0] * 3, r2f_full.shape[1] * 3),
    )

    print(r2f_full_expanded.shape)

    with h5py.File(
        workspace_path + "CT_bezier_r2f_with_dirichlet_expanded.hdf5", "w"
    ) as f:
    ...
    with h5py.File(workspace_path + "CT_bezier_all_matrices.hdf5", "w") as f:
        ...
        f.create_dataset(
            "A_proj_triplets/cols", data=r2f_full_expanded.col.astype(np.int32)
        )
        f.create_dataset(
            "A_proj_triplets/rows", data=r2f_full_expanded.row.astype(np.int32)
        )
...
def build_full_expanded_bezier_hard_constraint_matrix(
    workspace_path,
    tri_to_tet_index_mapping_file,
    bezier_cons_mat_file,
    linear_tetmesh_file,
    ...
    bezier_cons_matrix = scipy.io.mmread(
        bezier_cons_mat_file
    )  # this is already expanded to xyz
    ...
    r2f_full_expanded = scipy.sparse.coo_array(
        (r2f_full_data, (r2f_full_row, r2f_full_col)),
        shape=(v.shape[0] * 3, all_ind_ids.shape[0]),
    )
