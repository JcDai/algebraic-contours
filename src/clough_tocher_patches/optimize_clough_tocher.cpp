...
CloughTocherOptimizer::generate_laplacian_stiffness_matrix() const
{
	...
	for (int fijk = 0; fijk < num_faces; ++fijk)
	{
		...
		assemble_local_laplacian_siffness_matrix(face_chart.face_uv_positions, nodes,
																						 stiffness_matrix_trips);
	}
...
Eigen::SparseMatrix<double>
CloughTocherOptimizer::generate_laplace_beltrami_stiffness_matrix(const std::vector<Eigen::Vector3d> &bezier_control_points) const
{
	// assemble IJV matrix entries
	const auto &affine_manifold = get_affine_manifold();
	std::vector<Triplet> stiffness_matrix_trips;
	...
	for (int fijk = 0; fijk < num_faces; ++fijk)
	{
		...
		std::array<std::array<int64_t, 10>, 3> nodes =
				get_micro_triangle_nodes(fijk);
		assemble_local_laplace_beltrami_siffness_matrix(bezier_control_points, nodes,
																										stiffness_matrix_trips);
	}
...
void CloughTocherOptimizer::initialize_ind_to_full_matrices()
{
				// TODO: Would be better to avoid the uneccesary construction of a
surface Eigen::SparseMatrix<double> fit_matrix; Eigen::SparseMatrix<double>
energy_hessian; Eigen::CholmodSupernodalLLT<Eigen::SparseMatrix<double>>
												energy_hessian_inverse;
				OptimizationParameters optimization_params;
				CloughTocherSurface ct_surface(V, affine_manifold, optimization_params,
																																																																				fit_matrix, energy_hessian,
...
diff.col(i).norm()); break;
				...
				spdlog::info("Matrix entries are {} and {}", _ind2full.coeffRef(153,
153), ind2full.coeffRef(153, 153));
